...
export class MediaListComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild("confirmPublishBtn") confirmPublishBtn: ElementRef;
  ...
  ngOnInit() {
    ...
    this.dtOptions = {
      ajax: (dataTablesParameters: any, callback: any) => {
        that.http
          .get<any>(`${this.baseURL}/media?draft=false`)
          .subscribe((resp: any) => {
            callback({
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data: any, type: any, full: any, meta: any) {
            var id = full["_id"];
            return (
              '<input type="checkbox" type="checkbox" id=' +
              id +
              " data-id=" +
              full.id +
              '   class="dt-checkboxes  tblChk form-check-input">'
            );
          },
          checkboxes: {
            selectAllRender:
              '<input type="checkbox" class="form-check-input tblChkAll">',
          },
        },
        {
          targets: 2,
        ...
        },
        {
          ...
          render: function (data: any, type: any, full: any, meta: any) {
            ...
            if(full["publishedDate"]!=null){
              const date = new Date(full["publishedDate"]);
        ...
        },
        {
          ...
          render: function (data: any, type: any, full: any, meta: any) {
            var $user_img = full["avatar"],
              $post = full["post"];
            var $name =
              full["creator"]["firstName"] + " " + full["creator"]["lastName"];
            ...
            if ($user_img) {
              ...
              var $name =
                full["creator"]["firstName"] +
                " " +
                full["creator"]["lastName"];
    ...
    window.functions.publishBulkEntity = this.publishBulkEntity.bind(this);
  }
  ...
  publishBulkEntity() {
    // console.log('Button clicked for ' + item);
    this.confirmPublishBtn.nativeElement.click();
    // this.selectedMedia=[item]
  }
  editEntity(item) {
    console.log("Button clicked for " + item);
  ...
  Bulk Action
    ...
    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
      ...
      <li><a class="dropdown-item" href="javascript:;" onclick="functions.publishBulkEntity()">Publish</a></li>
    </ul>
  </div>
      `
          );
  ...
  deleteConfirm() {
    this.mediaSerice.deleteMedia(this.selectedMedia).subscribe((res) => {
      // reload page
      console.log(res);
      this.toastService.show("Selected Items Deleted", {
  ...
  publishConfirm() {
    this.mediaSerice.publishMedia(this.selectedMedia).subscribe((res) => {
      // reload page
      console.log(res);
      this.toastService.show("Selected Items Deleted", {
        classname: "bg-success text-dark",
